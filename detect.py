...
def detect(opt):
    ...
    with open(save_dir / "info.txt", 'w') as f:
        ...
        s += f"Conf-threshold: {opt.conf_thres}\n"
        f.write(s)
    ...
    submission = None
    if opt.save_csv:
        submission = pd.read_csv(Path('./data/submission.csv'))
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if submission is not None:
                entries = submission[submission['image_name'] == p.stem].index
                if len(entries) == 1:
                    submission_entry = entries[0]
            pred_str = []  # predict string for submission
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if submission_entry >= 0:
                        pred_str.append(' '.join([str(min(max(int(v.item()), 0), 1024)) for v in xyxy]))
    ...
    if submission is not None:
        submission.to_csv(save_dir / 'submission.csv')
    ...
    with open(save_dir / "info.txt", 'a') as f:
        s = f"Images: {len(dataset)}\n"
        s += f"Args: {opt}\n"
        f.write(s)
    with open(save_dir / "names.txt", 'w') as f:
        for n in img_names:
            f.write(n + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-csv', action='store_true', help='save prediction to submission.csv')
